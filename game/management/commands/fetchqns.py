...
def configure_logger(enable_logging, log_to_file):
    ...
    if enable_logging:
        ...
        handler = logging.FileHandler("trivia_fetch.log") if log_to_file else logging.StreamHandler()
...
class Command(BaseCommand):
    ...
    async def _GET_query_initializer(self):
        ...
        async def generator(amt, cat, diff, options_type, debug=False):
            ...
            self.logger.warning(f"TYPE: {type(amt)}, {type(cat)}, {type(diff)}, {type(options_type)}")
            
            parameters = list(global_parameters)
            amt = str(amt) if (
                isinstance(amt, int) and (10 <= amt <= 50)
            ) else None
            cat = str(cat) if (
                (isinstance(cat, int) or isinstance(cat, str)) and 
                (cat in tuple(categories.keys()))
            ) else None
            diff = diff if (
                isinstance(diff, str) and 
                (diff in difficulty)
            ) else None
            options_type = options_type if (
                isinstance(options_type, str) and 
            ...
            if 'category' in parameters:
                ...
                if 'difficulty' not in parameters:
                    diff = 'total'
                args[0] = str(category_count[diff] - 1) if (
                    int(amt) >= category_count[diff]
                ) else args[0]
    ...
    async def _fetch_questions(self, api_urls):
        ...
        async with aiohttp.ClientSession() as session:
            for api_url in api_urls:
                ...
                if rc != 0:
                    self.logger.warning(" | ".join([api_url, str(rc), response_code_statuses[rc]]))
                else:
    ...
    def handle(self, *args, **options):
        ...
        amount = list(range(10, 20)) if (
            amount == 0
        ) else [amount]

        category = self._categories if (
            category == "any"
        ) else list(map(lambda x: int(x), category))

        difficulty = ["easy", "medium", "hard", "any"] if (
            difficulty == "all"
        ) else [
            self._mapped_difficulties[d] for d in difficulty
        ]
