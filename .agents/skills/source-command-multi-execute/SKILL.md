---
name: "source-command-multi-execute"
description: "Run the migrated source command `multi-execute`."
---

# source-command-multi-execute

Use this skill when the user asks to run the migrated source command `multi-execute`.
...
Use the task details supplied by the user as the execution input.
...
**Execute Task**: use the task details supplied by the user.
...
$source-command-multi-execute implement user authentication based on previous plan
```
...
Migrated from source command `multi-execute` into a Codex skill. Invoke it as `$source-command-multi-execute` or ask for multi-model execution in plain language.

Provider argument placeholders have been rewritten into natural-language instructions where they controlled command input.

Template variables in shell examples are placeholders for concrete values chosen at runtime; do not pass them literally.
