...
Use this skill when the user asks to run the migrated source command `python-review`.
...
```text
User: $source-command-python-review
...
- Run the relevant Python tests first to ensure they pass
- Use `$source-command-code-review` for non-Python specific concerns
- Use `$source-command-python-review` before committing
- Fix static analysis failures before approving changes
...
- Agent: `.codex/agents/python-reviewer.toml`
- Skills: `.agents/skills/python-patterns/`, `.agents/skills/python-testing/`
...
Migrated from source command `python-review` into a Codex skill. Invoke it as `$source-command-python-review` or ask for a Python review in plain language. The original slash-command behavior is preserved as prose.
